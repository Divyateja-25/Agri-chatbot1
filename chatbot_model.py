...
def get_response(user_input, lang=None):
    """
    ...
    # --- Step 0: Greetings ---
    user_input_lower = user_input.lower()
    for l, greet_list in greetings.items():
        if any(greet.lower() in user_input_lower for greet in greet_list):
            return random.choice(greeting_responses.get(l, greeting_responses["en"]))

    # --- Step 0b: Farewells ---
    for l, bye_list in farewells.items():
        if any(word.lower() in user_input_lower for word in bye_list):
            return random.choice(farewell_responses.get(l, farewell_responses["en"]))

    # --- Step 1: Offline KB ---
    response = get_offline_response(user_input, lang=lang)
    if response:
        return response
